...
To get a 2-dim $\sigma$-model we then need to consider the CS theory on $\Sigma\times[0,1]$, with the $V_+$-boundary condition on $\Sigma\times\{0\}$, and with a topological boundary condition on $\Sigma\times\{1\}$ given by a Lagrangian Lie subalgebra $\h\subset\g$. (We also need to suppose that the inner product on $\g$ is positive definite on $V_+$ and negative definite on $V_-$.) 
$$
\begin{tikzpicture}
...
The (naive) space of fields of the CS theory is $\Omega^1(B^3)\otimes\g$. The Lie algebra $\g$ will play a very little role in the propagator, so for the moment we shall suppose that the space of fields is $\Omega^1(B^3)$ and restore $\g$ only at the  end of this section.
...
On the other hand, to define $w^{(\ell)}_{\Gamma}$ and $T_{\Gamma}$ for a signed $\Gamma$ individually (i.e.\ not just their product $\alpha^{(\ell)}_{\Gamma}$) we need additional choices on $\Gamma$, e.g.\ a cyclic order at each internal vertex to fix the signs of $w^{(\ell)}_{\Gamma}$ and $T_{\Gamma}$. To make things explicit, we shall always use the anticlockwise order in the diagrams that we draw.
\end{rem}
...
\bibitem{R} Roytemberg (Courant model)
