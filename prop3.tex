...
\title[RG flow on Chern-Simons boundary]{RG flow on Chern-Simons boundary, generalized Ricci tensor, and Poisson-Lie T-duality {Jan's version}}
%\title{Generalized Ricci tensor and the boundary of Chern-Simons theory}
\thanks{Supported in part by the  NCCR SwissMAP of the Swiss National Science Foundation. D.\ R.\ Youmans was further supported by the National Center for Competence in Research (NCCR) SwissMAP grant 178794}
\author{Ján Pulmann}
...
The chiral boundary condition, introduced by Witten in his famous work \cite{W}, is probably the most useful boundary condition of Chern-Simons theory. Roughly speaking, it needs the boundary $\Sigma$ of the 3-manifold $M$ to be a Riemann surface and it requires the connection $A\in\Omega^1(M,\g)$ to be in
$\Omega^{1,0}(\Sigma,\g)$
when restricted to $\Sigma$. The boundary field theory thus obtained is the chiral WZW model.

There is a straightforward generalization of the chiral boundary condition, introduced in \cite{S}, linking Chern-Simons theory to other 2-dim $\sigma$-models and to Poisson-Lie T-duality. Given an orthogonal splitting $\g=V_+\oplus V_-$ to a pair of vector subspaces, the corresponding $V_+$-boundary condition requires the restriction of $A$ to $\Sigma$ to be in $$\Omega^{1,0}(\Sigma,V_+)\oplus\Omega^{0,1}(\Sigma,V_-).$$
...
\begin{figure}[h]
	\includesvg[scale=0.5]{sketch.svg}
...
The element $-T_{D}\in V_+\otimes V_-$ is known as the \emph{generalized Ricci tensor} of the generalized metric $V_+\subset\g$ \cite{CSW,G,SV,???}. The 1-loop conformality condition is thus the vanishing of the generalized Ricci tensor. {\color{teal} Note that $T_{D} = 0$ for the usual chiral boundary condition, where $V_{+} = \g$. } %{\color{red} CITE \cite{SST} PROPERLY}
...
{\color{teal} The correlation functions $G^{(\ell)}_{k}(x_{1}, \dots, x_{k})$ then can be computed from 
$S^{(\ell)}_\text{eff}$ using only tree diagrams \cite{Weinberg}.}
...
\end{tikzpicture}
$${\color{teal}
The expression associated to this graph is 
\[ \frac{i\hbar}{2} \int_{q_{1}, q_{2} \in \cf_{2}(B^3)}  A^{i}(q_{1}) A^{j}(q_{2}) \Theta_{\ell}(z_{1}, z_{2})  \frac{d z_{1} dz_{2}}{ 2\pi i (z_{1}-z_{2})^{2}}\frac{d \bar{z}_{1} d\bar{z}_{2}}{ -2\pi i (\bar{z}_{1}-\bar{z}_{2})^{2}} \langle T_{D}, e_{i} \otimes e_{j} \rangle \]
where $z_{1}, z_{2}$ are functions of $q_{1}$ and $q_{2}$ and $A = A^{i}\otimes e_{i} \in \Omega(B^{3})\otimes \g$. To get the part that diverges as $\ell\to 0$, we use the following lemma which can be proved by a straightforward calculation (see Appendix). The term $r^{*}(\Theta_{\epsilon\ell_1}-\Theta_{\epsilon\ell_2})$ restricts $z_{2}$ to a small annulus around $z_{1}$.
} 

{\color{gray}To get its divergent part, we use the following lemma which can be proved by a straightforward calculation (see Appendix).}
...
\bibitem{R} Roytenberg (Courant model)
